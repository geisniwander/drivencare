...
async function createPatient({ name, email, password}) {
  ...
  const doctorCount = (await doctorsRepository.findDoctorByEmail(email)).rowCount;
  if (rowCount || doctorCount) throw errors.duplicatedEmailError(email);

  const hashPassword = await bcrypt.hash(password, 10);
  await patientsRepository.createPatient({ name, email, password: hashPassword});
}
...
async function signin({ email, password }) {
  ...
  const token = jwt.sign({ patient_id: patient.id }, process.env.SECRET_KEY, { expiresIn: 86400 }); // A chave secreta é um hash SHA-256
